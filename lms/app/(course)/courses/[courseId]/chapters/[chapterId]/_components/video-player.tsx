...
export const VideoPlayer = ({ playbackId, courseId, chapterId, nextChapterId, isLocked, completeOnEnd, title } : VideoPlayerProps) => {
    ...
    const onEnded = async () => {
        ...
        {
            toast.error("Something Went Wrong While Completing the Course, please contact admin for solution...!");
        }
    }
    ...
    return (
        <>
            <div className="relative aspect-video">
                ...
                {
                    !isLocked && (
                        <MuxPlayer title={ title } className={ cn( !isReady && "hidden" ) } onCanPlay={ onCanPlay } onEnded={ onEnded } autoPlay playbackId={ playbackId } />
                    )
                }
            </div>
        </>
